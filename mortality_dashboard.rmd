---
title: "America's Health Divide: Mortality Patterns Across HHS Regions"
...
--------------------------------
1. Put "USRegionalMortality.csv" in the same folder as this .Rmd (or update
   the path in the "data-loading" chunk below).
2. Click into a chunk and press Ctrl+Shift+Enter (Cmd+Shift+Enter on Mac) to
   run it, or use the green "Run" arrow in the chunk's top-right corner.
   Run the chunks in order, top to bottom.
3. Two packages were used in the original script but never loaded:
   `bslib` (for bs_theme) and `shinydashboard` (for valueBox /
   valueBoxOutput). I added them to the "libraries" chunk below so the app
   will actually run - install them first if you don't have them:
   install.packages(c("bslib", "shinydashboard"))
4. The last chunk launches the app. Run it last, after every chunk above it.
-->
...
### 2a. Load the mortality data

```{r data-loading}
mortality_data <- read_csv("USRegionalMortality.csv", show_col_types = FALSE)
...
### 2c. Join data with region metadata
